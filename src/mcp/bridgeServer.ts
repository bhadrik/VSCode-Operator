...
const SERVER_INSTRUCTIONS = [
  ...
  "When connected through the fixed proxy endpoint, call vscode_workspace_status first to discover registered workspacePath values.",
...
const USAGE_GUIDE_TEXT = [
  ...
  "5) Through the proxy endpoint, call vscode_workspace_status first to list registered workspace bridges; then pass a returned workspacePath to workspace-specific tools.",
  ...
  "13) For workspace-specific requests in multi-workspace mode, include workspacePath in tool arguments or in resource URI query (e.g. vscode-operator://tools?workspacePath=<abs path>). An explicit workspacePath overrides any proxy session affinity.",
  "",
  "Parameter hints:",
